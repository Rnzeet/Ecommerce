...
router.post("/upload-image", upload.single("image"), async (req, res) => {
  try {
    ...
    if (uploadError) return res.status(400).json({ message: uploadError.message || "Image upload failed", details: uploadError });
...
router.post("/add-product", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("products")
      .insert([{ name, price: parseFloat(price), image, category, description }])
      .select();
...
router.put("/update-product/:id", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("products")
      .update({ name, price: parseFloat(price), image, category, description })
      .eq("id", id)
      .select();
    if (error) return res.status(400).json({ message: error.message, details: error.details });
